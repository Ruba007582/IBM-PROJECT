...
Original file is located at
    https://colab.research.google.com/drive/14fRJmB4X62wS0Z2Q-y82WI0zQXfkxU-i
"""
...
# ⚡ Generate AI Response with deterministic settings
...
# ⚡ Test Python Code Execution (Sandboxed)
...
def requirement_analysis(pdf_file, prompt_text):
    ...
    analysis_prompt = (
        "🔍 Analyze the following and extract key software requirements:\n\n"
        f"{content}\n\n"
        "Organize into:\n"
...
# 💻 Code Generation Function with Validation
def code_generation(prompt, language):
    code_prompt = (
        f"🛠️ Generate a complete, runnable {language} code based on the following requirement. Include all necessary imports and functions:\n\n"
        f"{prompt}\n\n"
        "Code:"
    )
    ...
    if language.lower() == "python":
        ...
        if not is_valid:
            code = f"⚠️ Syntax error detected:\n{message}\n\n{code}"
        else:
...
with gr.Blocks(css=custom_css) as app:
    gr.Markdown("## 🤖 Smart SDLC: AI Code Analysis & Generator")

    with gr.Tabs():
        with gr.TabItem("📑 Code Analysis"):
            with gr.Row():
                with gr.Column():
                    pdf_upload = gr.File(label="📂 Upload PDF (.pdf)", file_types=[".pdf"])
                    prompt_input = gr.Textbox(
                        label="📝 Or type requirements manually",
                ...
                with gr.Column():
                    analysis_output = gr.Textbox(label="✅ Analyzed Requirements", lines=20)
